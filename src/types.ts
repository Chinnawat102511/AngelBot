...
export type StrategyConfig = {
  ...
  ma?: { enabled?:boolean; type?: "EMA"|"SMA"|"WMA"|"HMA"; length?: number; bias?: "NONE"|"UP"|"DOWN" };
...
export type MgConfig = {
  ...
  base: number; mult: number; steps: number;
  customList: number[];
...
export type Trade = {
  ts:number; asset:string; tf:TF; side:"CALL"|"PUT";
  amount:number; mgStep:number; result:"WIN"|"LOSE"|"DRAW";
  payout:number; pnl:number; strategy:string;
};

export type Persisted = {
  tf: TF; assets: string[]; mg: MgConfig;
  strategy1m: StrategyConfig; strategy5m: StrategyConfig;
  filters: Filters; risk: Risk; orderIntervalSec: number;
